...
def load_data_objects():
    ...
    return {
        ...
        "risk_band_summary": pd.read_csv(ARTIFACT_DIR / "risk_band_summary.csv"),
        "decision_band_summary": pd.read_csv(ARTIFACT_DIR / "decision_band_summary.csv"),
...
def assign_risk_band(pd_value: float, band_summary: pd.DataFrame) -> str:
    for _, row in band_summary.iterrows():
        if row["min_pd"] <= pd_value <= row["max_pd"]:
            return row["risk_band"]
    if pd_value < band_summary["min_pd"].min():
        return band_summary.sort_values("min_pd").iloc[0]["risk_band"]
    return band_summary.sort_values("max_pd").iloc[-1]["risk_band"]


def assign_decision_band(risk_band: str) -> str:
    mapping = {
        "Very Low": "Approve",
        "Low": "Approve",
        "Medium": "Review",
    ...
    return mapping.get(risk_band, "Review")