...
def load_data_objects():
    ...
    for key, filename in optional_csvs.items():
        data[key] = read_optional_csv(filename)
...
def format_pd(x, decimals: int = 4) -> str:
    if pd.isna(x):
        return "-"
...
def build_existing_borrower_sequence(sk_id: int, data_dict: dict):
    ...
    borrower_df = (
        seq_panel[seq_panel["SK_ID_CURR"] == sk_id]
        .copy()
    ...
    if "time_mask" in borrower_df.columns:
        mask = borrower_df[["time_mask"]].values.astype(bool)
    else:
        mask = np.ones((len(borrower_df), 1), dtype=bool)
...
def score_borrower(static_processed, embedding, xgb_model, platt, emb_cols):
    ...
    fused = pd.concat(
        [static_processed.reset_index(drop=True), emb_df],
        axis=1,
    )
...
def assign_decision_from_grade(grade: str) -> str:
...
def assign_grade_group(grade: str) -> str:
    if grade in ["AAA", "AA", "A"]:
